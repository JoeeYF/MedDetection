...
'''
python frocwrtdetpepchluna16_medtk_Luna2016.py --config path/.../cfg_sub_model_3_nodule_det_rcnn_v_base_5 --fold 9
'''
...
model_dirname = os.path.basename(args.config)  # 'cfg_sub_model_3_nodule_det_rcnn_v_base_5'
epochs = range(10, 160, 10)

cur_dir = os.path.abspath(os.path.dirname(__file__))
annotations_filename = cur_dir + '/annotations/annotations.csv'  # path for ground truth annotations for the fold
annotations_excluded_filename = cur_dir + '/annotations/annotations_excluded.csv'  # path for excluded annotations for the fold

for epoch in epochs:
    print(epoch)
    output_dir = f'{cur_dir}/folds_{SUFFIX}/{model_dirname}/{FOLD}/{epoch}ep'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    seriesuids_filename = f'{output_dir}/fold{FOLD}_seriesuids.csv'  # path for seriesuid for the fold
    results_filename = f'{output_dir}/fold{FOLD}.csv'

    results_path = os.path.abspath(f'work_dirs/{model_dirname}/infer_results_{FOLD}_{epoch}ep/')
    datapath = '/home/qiao/Desktop/Detection/Luna2016/train_images_test/subset' + str(FOLD) + '/'
    print(results_path)
    ...
    def convertcsv_medtk(bboxfname, bboxpath, detp):  # using Luna2016
        uid = bboxfname.split('_')[0]

        itkimage = sitk.ReadImage(datapath + uid + '_iso_crop_img.nii.gz')
        # numpyImage = sitk.GetArrayFromImage(itkimage)
        ...
        #xyz, xyz, cls, score  -> score, zyx, diameter
        diameter = np.mean(pbb[:, 3:6] - pbb[:, :3], axis=1, keepdims=True)  # add new 9 15
        pbb = np.concatenate([pbb[:, [-1]], (pbb[:, 3:6] + pbb[:, :3])[:, ::-1] / 2, diameter], axis=1)
        print(pbb.shape)

        # pbb = np.array(pbb[pbb[:, 0] > 1 / (1 + np.exp(-detp))])
        pbb = np.array(pbb[pbb[:, -1] > 3])  # add new 9 15
        # print(pbbold.shape)
        ...
        for nk in range(pos.shape[0]):  # pos[nk, 2], pos[nk, 1], pos[nk, 0]
            rowlist.append([uid, pos[nk, 2], pos[nk, 1], pos[nk, 0], pbb[nk, 0]])
    ...
    seriesuids = []
    for i, filename in enumerate(sorted(glob(results_path + '/*_det.npy'))):
        print(i, filename)
    ...
    pd.DataFrame(data=seriesuids).to_csv(seriesuids_filename, index=False, header=False)
