...
model_dirname = 'cfg_sub_model_3_nodule_det_rcnn_v2_base_1'
epochs = range(50, 160, 10)

for epoch in epochs:
    output_dir = './folds_{0}/{1}/{2}/{3}ep'.format(SUFFIX, model_dirname, FOLD, epoch)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    seriesuids_filename = '{0}/fold{1}_seriesuids.csv'.format(output_dir, FOLD)  # path for seriesuid for the fold
    results_filename = '{0}/fold{1}.csv'.format(output_dir, FOLD)

    results_path = '/home/timothy/PycharmProjects/MedToolkit/work_dirs/{}/infer_results_{}_{}ep/'.format(model_dirname, FOLD, epoch)
    ...
    def convertcsv_medtk(bboxfname, bboxpath, detp):  # using Luna16
        uid = bboxfname.split('_')[0]
        sliceim, origin, spacing, isflip = load_itk_image(datapath + '/' + uid + '.mhd')
        origin = np.load(sideinfopath + uid + '_origin.npy', mmap_mode='r')
        spacing = np.load(sideinfopath + uid + '_spacing.npy', mmap_mode='r')
