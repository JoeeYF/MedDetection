...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    for seriesuid in seriesUIDs:
        ...
        for noduleAnnot in noduleAnnots:
            ...
            if noduleAnnot.state == "Included":
                ...
                if found:
                    ...
                    FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%.9f" % (
                        seriesuid, 'TP', noduleAnnot.coordX, noduleAnnot.coordY, noduleAnnot.coordZ,
                        float(noduleAnnot.diameter_mm), float(candidate.CADprobability)))
                    candTPs += 1
                else:
                    candFNs += 1
                    ...
                    FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%s" % (
                        seriesuid, 'FN', noduleAnnot.coordX, noduleAnnot.coordY, noduleAnnot.coordZ,
                        float(noduleAnnot.diameter_mm), "NA"))
        ...
        for key, candidate3 in candidates2.items():
            ...
            excludeList.append(False)
            FROCtoNoduleMap.append("%s,%s,%s,%s,%s,%.9f,%.9f" % (
                seriesuid, 'FP', candidate3.coordX, candidate3.coordY, candidate3.coordZ,
                -1, float(candidate3.CADprobability)))
    ...
    if int(totalNumberOfNodules) > 0:
        ...
        ax.xaxis.set_ticklabels([str(i) for i in [0.125, 0.25, 0.5, 1, 2, 4, 8]])
        ax.yaxis.set_ticks(np.arange(0.3, 1, 0.1))
    ...
    return frvvlu / 7, fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up
...
def noduleCADEvaluation(annotations_filename, annotations_excluded_filename, seriesuids_filename, results_filename,
    ...
    froc, fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up = evaluateCAD(
...
if __name__ == '__main__':
    ...
    # noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir='./paper')

    # part of results
    SUFFIX = 'clean'
    FOLD = 9
    seriesuids_filename = 'folds_{0}/{1}/fold{1}_seriesuids.csv'.format(SUFFIX, FOLD)
    ...
    noduleCADEvaluation(annotations_filename, annotations_excluded_filename, seriesuids_filename, results_filename,
                        outputDir='folds_{0}/{1}'.format(SUFFIX, FOLD))

    # results_filename = './paper/3DRes18FasterR-CNN.csv'
    # noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir='./folds_paper/9')
    # print "Finished!"
