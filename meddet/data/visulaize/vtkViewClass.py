...
class MedicalImageWindow:
    ...
    def __init__(self):
        ...
        self.renWinInteractor.SetRenderWindow(self.renWin)  # 把上面那个窗口加入交互操作
...
class MedicalImageViewer:
    def __init__(self, renWin, renWinInteractor, render):
        ...
        self.volumeProperty_src = vtk.vtkVolumeProperty()  # 创建vtk属性存放器,向属性存放器中存放颜色和透明度
        self.volumeProperty_seg = vtk.vtkVolumeProperty()  # 创建vtk属性存放器,向属性存放器中存放颜色和透明度
    ...
    def addSrc(self, numpyImage_src, spacing=(1.0, 1.0, 1.0)):
        ...
        ctfun.AddRGBPoint(self.minGrayValue, 0.5, 0.0, 0.0)
        ctfun.AddRGBPoint(self.maxGrayValue, 0.9, 0.2, 0.3)
        ...
        volumeMapper_src.SetInputData(self.extractVOI_src.GetOutput())  # 向映射器中输入数据：shifter(预处理之后的数据)
...
def vtkWindowViewNotebook(numpyImage, spacing=(1.0, 1.0, 1.0)):
    ...
    os.system(f'{sys.executable} \"{os.path.dirname(__file__)}/tmp_func.py\" 2')
    print('closing')