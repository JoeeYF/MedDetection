...
from meddet.model.nd import AvgPoolNd
from meddet.model.blocks import BottleneckNd, ConvNormAct
...
class BasicResBlock(BlockModule):
    ...
    def __init__(self,
        ...
        self.dim = dim
        # main path
        self.conv1 = ConvNormAct(self.dim,
                                 in_channels,
                                 in_channels,
                                 kernel_size=3,
                                 padding=1,
                                 bias=False)
        self.conv2 = ConvNormAct(self.dim,
                                 in_channels,
                                 out_channels,
                                 kernel_size=1,
                                 bias=False,
        ...
        # identity path
        self.conv_identity = ConvNormAct(self.dim,
                                         in_channels,
                                         out_channels,
                                         kernel_size=1,
                                         do_act=False)
...
class DoubleBBoxHead(ComponentModule):
    def __init__(self,
                 dim,
                 num_classes,
                 in_channels,
        ...
        super(DoubleBBoxHead, self).__init__()
        ...
        self.fc_channels = 1024
        self.flatten_features = self.mid_channels * np.prod(np.array(self.roi_output_size))
        ...
        self.extractors = nn.ModuleList([RoIAlign(self.roi_output_size, 1 / stride, 1) for stride in [4]])
    ...
    def _init_layers(self):
        self.res_block = BasicResBlock(self.dim, self.in_channels, self.conv_out_channels)

        self.conv_branch = nn.Sequential(
            BottleneckNd(self.dim, self.conv_out_channels, self.fc_channels // BottleneckNd.expansion),
        )
        self.avg_pool = AvgPoolNd(self.dim)(kernel_size=self.roi_output_size)

        self.fc_branch = nn.Sequential(
            nn.Linear(self.flatten_features, self.fc_channels),
    ...
    def _bbox_forward(self, feats, rois):
        ...
        x_cls, x_reg = roi_features.clone(), roi_features.clone()

        x_conv = self.res_block(x_reg)
        x_conv = self.conv_branch(x_conv)
        x_conv = self.avg_pool(x_conv)
        x_conv = x_conv.view(x_conv.size(0), -1)
        reg_out = self.roi_reg(x_conv)

        x_fc = x_cls.view(x_cls.size(0), -1)
        x_fc = self.fc_branch(x_fc)
        cls_out = self.roi_cls(x_fc)
